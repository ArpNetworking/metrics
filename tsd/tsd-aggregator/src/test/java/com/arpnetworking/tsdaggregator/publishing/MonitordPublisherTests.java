...
public class MonitordPublisherTests {
    ...
    public void testShortPeriodDoesNotPost() {
        ...
        data[0] = new AggregatedData(new SumStatistic(),
            "service_name",
            "host",
            "set/view",
            2332d,
            DateTime.now()
                .minus(Period.minutes(1)),
            Period.seconds(1),
            new Double[]{});
        publisher.recordAggregation(data);
    ...
    public void testExceptionOnRequest() {
        ...
        context.checking(new Expectations() {{
            try {
                one(client).execute(with(any(HttpUriRequest.class)));
                will(throwException(new ClientProtocolException("big bad error")));
            } catch (IOException ignored) {
            }
        }});
    ...
    public void testMultipleAggregationValuesPostsOnce() {
        ...
        data[0] = new AggregatedData(new SumStatistic(),
            ...
            metric,
            2332d,
            DateTime.now().minus(Period.minutes(1)),
            period,
            new Double[]{});
        data[1] = new AggregatedData(new MeanStatistic(),
            serviceName,
            host,
            metric,
            1007d,
        ...
        mockResponse(context, client, 200, "OK", new BaseMatcher<HttpUriRequest>() {
            ...
            public boolean matches(Object item) {
                @SuppressWarnings(value = "unchecked") HttpPost request = ( HttpPost ) item;
    ...
    @Test
    public void testDoesNotRecordOldData() {
        Mockery context = new Mockery();
    ...
    public void testDoesNotCrashOnIOErrorsOnContent() {
        ...
        context.checking(new Expectations() {{
            try {
                ...
                will(returnValue(responseStatusLine));
                atLeast(1).of(responseStatusLine)
                    .getStatusCode();
    ...
    private void mockResponse(@Nonnull final Mockery context, final HttpClient client, final int statusCode, final String content,
                              final Matcher<HttpUriRequest> matcher) {
        final HttpResponse response = context.mock(HttpResponse.class);
        final HttpEntity responseEntity = context.mock(HttpEntity.class);
        final StatusLine responseStatusLine = context.mock(StatusLine.class);
        ...
        context.checking(new Expectations() {{
            try {
                one(client).execute(with(matcher));
                will(returnValue(response));
                allowing(response).getEntity();
                will(returnValue(responseEntity));
                allowing(response).getStatusLine();
